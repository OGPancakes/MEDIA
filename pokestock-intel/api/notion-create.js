...
const json = (res, status, response) => res.status(status).json(response);
...
const storeStatus = (score) => {
  if (Number(score) >= 80) return "Recent restock";
  if (Number(score) >= 60) return "Likely soon";
  return "Inactive";
};
...
export default async function handler(request, response) {
  ...
  try {
    if (!process.env.NOTION_API_KEY) {
      return json(response, 500, { error: "Missing NOTION_API_KEY." });
    }
    ...
    if (!page) {
      return json(response, 400, { error: "Unknown Notion create type." });
    }

    return json(response, 200, { id: page.id });
  } catch (error) {
    return json(response, 500, { error: error.message || "Unable to create Notion record." });
  }
}
